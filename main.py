...
@app.get("/api/memory-palace/batch-embedding-probe")
async def api_memory_palace_batch_embedding_probe():
    """临时探针：确认服务商支不支持「一次传多段文字」，以及返回顺序对不对。

    验证完就删。做法是拿三段互不相干的文字，先逐段单独求向量（当作标准答案），
    再一次性打包求一遍，然后看打包结果的第 i 个是不是真的对应第 i 段。

    判断标准用余弦相似度：同一段文字两次求向量应该几乎完全相同（≈1.0）。
    如果第 0 个对上了第 1 段，说明顺序错乱，批量绝对不能上。
    """
    if not MEMORY_ENABLED:
        return {"error": "记忆系统未启用"}
    cfg = _mp_embedding_config()
    if not cfg["ready"]:
        return {"ok": False, "error": "EMBEDDING_API_KEY / EMBEDDING_BASE_URL / EMBEDDING_MODEL 未填全"}

    # 三段刻意选得毫不相干，这样错位一眼就能看出来
    texts = ["今天下午我去超市买了西红柿和鸡蛋",
             "量子力学中的波函数坍缩至今没有公认解释",
             "我家那只橘猫昨晚把花瓶撞翻了"]
    headers = {"Authorization": f"Bearer {cfg['api_key']}", "Content-Type": "application/json"}
    out = {
    ...
    try:
        ...
        singles = []
        for t in texts:
            emb = await compute_memory_palace_embedding(t)
            if not emb:
                out["error"] = "连单条向量都拿不到，先解决单条问题"
                return out
            singles.append(emb)
        out["single_dim"] = len(singles[0])

        # 打包求。带不带 dimensions 各试一次，记下哪种能用
        bodies = []
        if cfg["dim"] > 0:
            bodies.append(("带 dimensions", {"model": cfg["model"], "input": texts, "dimensions": cfg["dim"]}))
        bodies.append(("不带 dimensions", {"model": cfg["model"], "input": texts}))

        async with httpx.AsyncClient() as client:
            for label, body in bodies:
                attempt = {"label": label}
                try:
                    resp = await client.post(cfg["endpoint"], headers=headers, json=body, timeout=30.0)
                    attempt["status"] = resp.status_code
                    if resp.status_code >= 400:
                        attempt["detail"] = resp.text[:300]
                        out["notes"].append(attempt)
                        continue
                    data = resp.json()
                    items = data.get("data") or []
                    attempt["returned_count"] = len(items)
                    if len(items) != len(texts):
                        attempt["detail"] = f"只回了 {len(items)} 条，期望 {len(texts)} 条"
                        out["notes"].append(attempt)
                        continue

                    # 服务商可能给每条带 index 字段，先看它自己怎么标的
                    raw_index = [it.get("index") for it in items]
                    attempt["reported_index"] = raw_index

                    batch = [it.get("embedding") for it in items]
                    if any(not b for b in batch):
                        attempt["detail"] = "有条目没有 embedding"
                        out["notes"].append(attempt)
                        continue
                    attempt["batch_dim"] = len(batch[0])

                    # 核心检查：批量第 i 条 vs 单独第 j 条的相似度矩阵
                    matrix = []
                    for i, b in enumerate(batch):
                        row = [round(_memory_palace_cosine(b, s), 4) for s in singles]
                        matrix.append(row)
                    attempt["similarity_matrix"] = matrix
                    # 每行最大值的位置应该正好是 i
                    best = [max(range(len(row)), key=lambda j: row[j]) for row in matrix]
                    attempt["best_match_for_each"] = best
                    diagonal = [matrix[i][i] for i in range(len(batch))]
                    attempt["diagonal"] = diagonal
                    aligned = best == list(range(len(texts))) and all(v > 0.99 for v in diagonal)
                    attempt["aligned"] = aligned

                    out["batch_accepted"] = True
                    out["notes"].append(attempt)
                    if aligned:
                        out["ok"] = True
                        out["order_correct"] = True
                        out["working_label"] = label
                        out["batch_dim"] = len(batch[0])
                        break
                    else:
                        out["order_correct"] = False
                except Exception as e:
                    attempt["detail"] = f"{type(e).__name__}: {e}"
                    out["notes"].append(attempt)

        if out["ok"]:
            out["verdict"] = f"可以批量：{out.get('working_label')}，顺序一致，{out['batch_dim']} 维"
        elif out["batch_accepted"] and out["order_correct"] is False:
            out["verdict"] = "危险：接受批量但返回顺序错位，不能上批量"
        elif not out["batch_accepted"]:
            out["verdict"] = "服务商不接受一次传多段，批量优化做不了"
        add_dashboard_log("info", f"🧪 批量向量探针：{out.get('verdict')}", category="mp-vector")
        return out
    except Exception as e:
