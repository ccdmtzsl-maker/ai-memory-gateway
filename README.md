...
- **主动消息** — `<自动触发>` 开头的触发消息只发给上游、不落库，历史里只留 AI 的主动发言
...
- **稀疏时间戳** — 历史与当前消息按时间间隔补时间戳，而非逐条硬打
  - 设置页「缓存分区」新增开关 `SPARSE_TIMESTAMP_ENABLED`，默认关闭
  - 首条、跨天打 `[07-29 周三 18:17]`（带星期），同天间隔满 15 分钟打 `[18:17]`，其余不打
  - 间隔满 6 小时追加一行「（距上次对话约 N 小时）」，只挂在后一条消息上，避免模型误判是自己拖延
  - assistant 消息同样参与打戳，为后续「特定 user 消息不落库」的主动消息场景留出时间线
  - 时间戳与正文之间空一行，降低模型模仿输出时间戳的概率
  - 已带 Operit 附件时间戳的消息不重复打戳，但间隔说明照补；间隔一律按 `created_at` 计算，与有无附件无关
  - 分区模式作用于 A 区与当前消息，B 区保持原有逐条紧凑戳；轮数不足降级到基础模式时同样生效
- **用户画像近期聊天补时间戳** — 画像材料里的近期聊天此前只有说话人和消息 ID，模型无法判断时间跨度
  - 复用同一套稀疏规则与阈值，时间戳独占一行，多条消息共享一个时间锚点
  - 每条对话线独立重置时间基准，不跨线计算间隔
  - 记忆提取材料本就带完整 `[YYYY-MM-DD HH:MM]`，无需改动
- **画像材料 update 模式规则反转** — update 改为收归档节点、排除事件盒 summary
  - 起因是部分记忆刚生成就被事件盒压缩归档，按原规则永远进不了画像
  - initial 模式不变，仍收未归档节点（含事件盒 summary）
  - update 模式近期聊天上限从 40 提到 50
- **修复** — A 区消息在打戳前就被剥掉 `created_at`，导致稀疏时间戳静默失效
- **主动消息不落库** — 正文以 `<自动触发>` 开头的 user 消息只发给上游，不写入对话记录
  - 配合 Operit 主动消息使用：历史里只留 AI 的主动发言，没有多余的触发指令
  - 标签严格限定在正文开头（允许前导空白），其他位置出现不生效；标签不剥离，整条原样发给模型
  - 连续触发一律追加，不做 re-roll 覆盖（多次触发是多次独立问候）
  - 流式与非流式、含工具调用的同步存储路径均已拦住，不会漏存 user
