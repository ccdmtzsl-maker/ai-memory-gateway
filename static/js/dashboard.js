...
const _SETTINGS_FIELDS = {
    ...
    int: ['MEMORY_PALACE_DEFAULT_LIMIT', 'MEMORY_PALACE_INJECTION_DEPTH', 'CACHE_PARTITION_X', 'CACHE_PARTITION_B_LIMIT', 'CACHE_PARTITION_EXTRACT_LIMIT', 'CACHE_PARTITION_WINDOW', 'EMBEDDING_DIM'],
    float: [],
    optionalFloat: ['CHAT_TEMPERATURE'],
    bool: ['MEMORY_ENABLED', 'KEYWORD_CONTEXT_ENABLED', 'CONTEXT_TEMPLATE_ENABLED', 'SPARSE_TIMESTAMP_ENABLED', 'CACHE_PARTITION_ENABLED', 'CACHE_PARTITION_KEEP_A_TOOLS', 'TOOL_CHAIN_DEBUG', 'FORCE_STREAM', 'PERF_DIAGNOSTIC_ENABLED', 'RESPONSE_TRANSFORM_ENABLED'],
    range: [],
    text: ['systemPrompt', 'dailyImpressionPrompt', 'RESPONSE_TRANSFORM_RULES', 'CONTEXT_TEMPLATE'],
};
...
function renderUserImpression() {
    ...
    el.innerHTML = '<div style="display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:12px;">' +
        '<div><div style="font-weight:800;">当前画像</div><div style="font-size:12px;color:var(--text-muted);margin-top:2px;">更新时间：' + uiEsc(_userImpressionCurrent.updated_at || '') + ' · 来源：' + uiEsc(_userImpressionCurrent.source_mode || '') + (_userImpressionCurrent.last_consumed_node_id ? ' · 待处理记忆：' + (_userImpressionCurrent.pending_memory_count || 0) + '条' : '') + '</div></div>' +
