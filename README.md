...
- **记忆宫殿** — 七房间架构（卧室/客厅/书房/用户房/自我房/阁楼/窗台），自动提取 + 事件盒打包 + 每日审视 + pgvector 语义搜索 + 导入导出
- **用户画像** — 自动生成并维护用户人格画像，增量更新（只看新增记忆，不全量重抽）
- **用户活跃元数据** — 手动统计长期活跃节律和主题分布，结果持久化保存，可通过 `{{user_activity_meta}}` 显式注入 prompt
- **日印象** — AI 对每天对话生成日记式总结，带标签和心情，支持自定义提示词
- **自动提取** — 对话结束后后台小模型提取记忆，参考材料复现当时真实召回状态
- **手动提取** — 从对话线或纯文本提取记忆，支持预览后确认
...
- **自定义系统消息** — 用户消息之后追加自定义 system 消息，支持模板变量排布
- **Operit 附件转换** — Operit 附件格式自动转为标准多模态内容
- **Operit 工具调用适配** — 工具调用消息格式适配，跨客户端兼容
- **非流正则转换** — 非流式响应可按正则规则做后处理转换
- **稀疏时间戳** — 按时间间隔补时间戳而非逐条硬打，减少噪音
- **主动消息不落库** — `<自动触发>` 开头的 user 消息只发给上游不写入记录

### 基础设施

- **全端点鉴权** — `GATEWAY_SECRET` 保护所有 API
- **设置面板** — 网页端管理所有配置，热更新无需重启
- **可视化日志** — 仪表盘实时查看记忆注入、提取、向量检索等过程日志
...
用户画像页面顶部提供独立的「用户长期活跃元数据」面板，不写入用户画像正文，也不调用 LLM。点击「统计活跃元数据」后统计并保存：

- **活跃节律** — 用户消息数、近 30/90 天活跃天数、活跃时段、最长连续活跃和最长沉默
- **主题分布** — 记忆宫殿房间占比和高频标签

统计结果按角色保存到 PostgreSQL，重新部署后仍然保留。面板进入时只读取已有快照，不会自动重算；再次点击按钮才会强制刷新。统计时段使用 `TIMEZONE_HOURS`，默认是 `UTC+8`。

如果要把这份结构化数据插入自定义 prompt，在模板中显式写入：

```text
{{user_activity_meta}}
```

变量只在显式出现时替换，不会自动注入所有 prompt；没有统计快照时会按需生成一次。
...
新增用户长期活跃元数据统计（活跃节律 + 主题分布），支持 Dashboard 手动刷新、PostgreSQL 持久化和 `{{user_activity_meta}}` prompt 变量；记忆提取提示词调整低重要性字段规则并移除示例

### v4.4（2026-08）

记忆宫殿向量检索迁移至 pgvector；记忆提取参考材料重构（真实召回 + 凑满 20 条 + 消息 ID 锚点 + 30 天清理）；设置保存优化（35 次往返降到 1 次）

### v4.3（2026-07）

稀疏时间戳；主动消息不落库；用户画像 update 模式规则调整

### v4.2（2026-07）

上下文模板（自定义系统消息）；图片归档拆分为独立模块；历史图片改用 URL 转发

### v4.1（2026-07）

图片归档（Cloudflare R2）；用户画像 v4.0（混合式结构 + 自选标签）

### v4.0（2026-07）

记忆宫殿（七房间架构）；日印象；用户画像；性能诊断；后端缓存

### v3.x（2026-05）

分区缓存；对话线管理；记忆向量搜索；设置面板；三层记忆架构

### v2.x（2026-03）

jieba 分词；完整上下文提取；记忆注入提示词优化
