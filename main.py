...
async def chat_completions(request: Request):
    ...
    if tool_messages:
        # 只把“当前轮”的工具消息算进去：以最后一条 assistant(tool_calls) 的 id 为准。
        # Operit 可能在 tool 后继续追加重复 user/历史 assistant，不能用“最后 assistant 位置”误杀本轮 tool。
        ...
        if last_tool_call_idx >= 0:
            expected_ids = {
                tc.get("id")
                for tc in messages[last_tool_call_idx].get("tool_calls", [])
                if tc.get("id")
            }
            tool_messages = [
                m for m in messages[last_tool_call_idx + 1:]
                if m.get("role") == "tool" and m.get("tool_call_id") in expected_ids
            ]
        else:
            tool_messages = []
        if tool_messages:
            print(f"🔧 检测到 {len(tool_messages)} 条当前工具结果消息")
