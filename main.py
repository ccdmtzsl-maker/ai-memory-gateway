...
def _shorten_external_timestamp(timestamp: str, history: list = None) -> str:
    """附件/proxy 自带时间戳也遵守同一天只显示 HH:MM。"""
    if not timestamp:
        return timestamp
    m = re.match(r'^\[([0-9]{2})-([0-9]{2}) ([0-9]{2}:[0-9]{2})\]
...
async def build_partitioned_messages(
    ...
    if current_user_msg:
        ...
        if attachment_time or proxy_time:
            time_text = _shorten_external_timestamp(attachment_time or proxy_time, history)
        if time_text:
