...
- 只输出 XML，不要代码块，不要额外文字。
- 正文里的引号可以直接写在 XML 文本节点里，不要使用 JSON。
...
async def generate_daily_impression_for_date(impression_date):
    ...
    try:
        ...
        def _strip_xml_fence(text: str) -> str:
            text = (text or "").strip()
            text = _re.sub(r"^```(?:xml)?\s*", "", text, flags=_re.I)
            text = _re.sub(r"\s*```$", "", text)
            return text.strip()

        def _extract_xml_tag(text: str, tag: str) -> str:
            m = _re.search(rf"<{tag}\b[^>]*>([\s\S]*?)</{tag}>", text, flags=_re.I)
            return _html.unescape(m.group(1).strip()) if m else ""

        raw_xml = _strip_xml_fence(raw)
        summary_text = _extract_xml_tag(raw_xml, "summary")
        mood_text = _extract_xml_tag(raw_xml, "mood")
        tags_block = _extract_xml_tag(raw_xml, "tags")
        tag_items = [
            _html.unescape(t.strip())
            for t in _re.findall(r"<tag\b[^>]*>([\s\S]*?)</tag>", tags_block, flags=_re.I)
            if t.strip()
        ]
        if not tag_items and tags_block:
            tag_items = [t.strip() for t in _re.split(r"[、,，\n]+", tags_block) if t.strip()]
