...
你必须完整输出所有应该存在的标签，包括旧内容和新更新。具体操作：

1. 对于数组类标签（如 likes, dislikes 等）：
   - 先读取旧画像中该标签的所有项
   - 结合新材料补充新发现的项
   - 输出完整的合并数组（旧项 + 新项，去重）
   - 按重要性排序，最重要的放前面

2. 对于字符串类标签（如 core_values, thinking_pattern 等）：
   - 先读取旧画像中该标签的内容
   - 结合新材料判断是否需要补充或修正
   - 输出完整的更新后内容（重写，不是追加）
   - 如果该标签实际包含多个独立项（如多条原则），应改用数组格式

3. 格式规范化：
   - 允许优化标签格式：旧画像如果用字符串分号分隔列表，应改成数组
   - 数组每项≤50字，段落≤150字

4. 标签的删除与保留：
   - 旧画像已有的标签，如果仍然成立，必须输出（即使新材料没提到）
   - 要删除某个旧标签，必须显式输出空字符串 ""
   - 完全不输出 = 保留旧内容（但尽量避免漏输出，应完整输出所有标签）

5. 新标签的添加：
   - 只有新材料给出足够证据时才添加新标签

6. summary 与 current_state 的更新节奏：
   - summary 保持长期连贯性，小幅渐进更新，不因近期闲聊大改
   - current_state 相反，就该大幅更新，反映当前状态切片

示例：
旧画像：
```json
"likes": ["看书", "游泳", "编程"],
"core_values": "做事要有始有终；重视效率"
```
...
```json
"likes": ["看书", "游泳", "编程", "徒步"],
"core_values": ["做事要有始有终", "重视效率", "重视公平"]
```

错误示范：
```json
"likes": ["徒步"],  ❌ 丢失了旧内容
"core_values": "做事要有始有终；重视效率；重视公平"  ❌ 应该改成数组
```
"""
