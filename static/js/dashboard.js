...
async function previewMemoryPalaceFromSelectedConversations(){mpPreviewDashLog('button handler entered');const checked=document.querySelectorAll('.conv-checkbox:checked');mpPreviewDashLog('checked boxes=' + checked.length);if(!checked.length){mpPreviewDashLog('no checked conversations', 'warn');return;}const sessionIds=Array.from(checked).map(cb=>cb.value);mpPreviewDashLog('sending preview API sessions=' + sessionIds.join(','));const p=convMpPanel();p.style.display='';p.innerHTML='<div style="padding:12px;color:var(--text-muted);">🧠 正在逐个对话线提取记忆预览...</div>';try{const r=await fetch('/api/memory-palace/extract-preview-sessions',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({session_ids:sessionIds,character_id:'default',limit:300})});mpPreviewDashLog('preview HTTP status=' + r.status);const d=await r.json();mpPreviewDashLog('preview API response status=' + (d.status || '') + ' groups=' + ((d.groups || []).length) + ' error=' + (d.error || ''));if(d.error||d.status==='error'){p.innerHTML='<div style="color:var(--error);padding:12px;">提取失败：'+escapeHtml(d.error||'未知错误')+'</div>';return;}renderConvMemoryPalacePreview(d.groups||[]);}catch(e){mpPreviewDashLog('preview request failed: ' + e.message, 'error');p.innerHTML='<div style="color:var(--error);padding:12px;">请求失败：'+escapeHtml(e.message)+'</div>';}}
...
const _SETTINGS_FIELDS = {
    ...
    int: ['MAX_MEMORIES_INJECT', 'MEMORY_EXTRACT_INTERVAL', 'MEMORY_PALACE_DEFAULT_LIMIT', 'CACHE_PARTITION_X', 'CACHE_PARTITION_WINDOW', 'EMBEDDING_DIM'],
    float: ['MIN_SCORE_THRESHOLD'],
    optionalFloat: ['CHAT_TEMPERATURE'],
    bool: ['MEMORY_ENABLED', 'MEMORY_PALACE_ENABLED', 'CACHE_PARTITION_ENABLED', 'MEMORY_VECTOR_ENABLED', 'FORCE_STREAM', 'RESPONSE_TRANSFORM_ENABLED'],
...
// Memory Palace conversation preview: global bindings for inline onclick debugging.
try {
