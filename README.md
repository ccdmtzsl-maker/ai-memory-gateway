...
- **记忆宫殿向量检索迁移至 pgvector** — 打分从 Python 逐条 JSON 解析 + 余弦计算改为数据库原生向量运算
  - 启动时自动检测 pgvector、建列、回填历史向量、建索引（HNSW 优先 / IVFFlat 兜底）
  - 四层回退保障：扩展不可用、列建失败、维度不符、单条未回填，均自动退回 Python 计算
  - 历史向量维度与列维度不符时不会崩，只告警；可用「补全向量」重算为当前维度
  - 向量统计接口显示 pgvector 就绪数：仪表盘点「查看向量数」即可确认检索走哪条路
- **设置保存优化** — 前端无条件提交全部字段，后端先读快照比对再批量写入
  - 35 次数据库往返降至 1 次多值 INSERT + ON CONFLICT（仅写变化项）
  - 未变化的设置跳过写入和缓存失效；运行时变量赋值一律照做，进程重启也不会丢失
  - 保存提示区分「已更新 N 项 / 跳过 M 项」
- **记忆提取参考材料重构** — 从「重新猜一遍相关记忆」改为「复现当时的真实召回状态」
  - 参考材料始终凑满 20 条，按三层优先级取：真实召回 → 语义检索 → 最近高重要
  - 真实召回：聊天时每轮注入的记忆都有记账，提取时按消息 ID 精确重建，不再靠时间窗猜
  - 轮次转圈取样：十几轮对话每轮都有代表，不再只拿最后几轮
  - 删除了 `last_accessed_at` 兜底排序：它会把后续对话的记忆混进旧对话的提取材料
  - receipts 查询加 session 隔离，不再串味；历史无 anchor 的记录回退时间窗
  - 记忆内容不再截断到 120 字；prompt 不再标注材料来源，避免干扰模型判断
  - receipts 保留 30 天自动清理，每次写入后后台触发，每天最多一次
- **GET 超时重试**（默认关闭）— 仪表盘 GET 请求 5 秒无响应自动中止重发，写操作不碰
