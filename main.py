...
from memory_palace_scoring import (
    _MEMORY_PALACE_BM25_WEIGHT,
    _MEMORY_PALACE_CANDIDATE_POOL,
    _MEMORY_PALACE_RECENCY_DECAY,
    ...
    _memory_palace_emotional_link_strength,
    _memory_palace_familiarity_bonus,
    _memory_palace_person_link_strength,
    _memory_palace_same_day_or_near,
    _memory_palace_score_rows,
    _memory_palace_should_promote,
    _memory_palace_vector_percentile,
    _memory_palace_vector_score_distribution,
)

from memory_palace_parsing import (
    TIMEZONE_HOURS,
    _MEMORY_PALACE_ALLOWED_MOODS,
    _MEMORY_PALACE_ALLOWED_ROOMS,
    _MEMORY_PALACE_ROOM_LABELS,
    _memory_palace_event_link_shadow_report,
    _memory_palace_float_or_none,
    _memory_palace_parse_args,
    _memory_palace_parse_summary_json,
    _memory_palace_resolve_fuzzy_date_references,
    _memory_palace_sample_evenly,
    ...
    parse_memory_palace_unpin_ids,
    safe_parse_digest_actions_json,
    safe_parse_memory_palace_json_array,
    serialize_memory_palace_correction_previews,
    split_memory_palace_extraction_snippets,
)
from tool_chain import (
    _dedupe_retained_use_package_chains,
    _drop_orphan_tool_messages,
    _extract_use_package_chains,
    _extract_xml_tool_calls_from_content,
    _flatten_retained_tool_chains,
    _log_tool_chain_snapshot,
    _map_tool_ids_to_db_pending,
    _normalize_incoming_xml_tool_messages,
    _normalize_tool_chains_by_id,
    _repair_tool_call_ids_by_adjacency,
    _retained_package_names,
)
from timestamps import (
    SPARSE_TS_GAP_MINUTES,
    _WEEKDAY_CN,
    _format_gap_note,
    _format_hm_duration,
    _last_message_dt,
    _memory_palace_format_node_line,
    _memory_palace_friendly_date,
    _memory_palace_indent,
...
from text_and_impression import (
    ...
    _merge_update_impression_tags,
    _ui_iso,
    _ui_preview_text,
    _user_impression_allocate_stage_quotas,
    _user_impression_allocate_update_stage_quotas,
    _user_impression_generation_key,
    _user_impression_memory_section_label,
    _user_impression_parse_embedding,
    _user_impression_select_stage_mmr,
    _user_impression_split_timeline,
    _user_impression_timeline_key,
    build_user_impression_generation_prompt,
    clean_user_message_for_log,
...
async def chat_completions(request: Request):
    ...
    if CACHE_PARTITION_ENABLED:
        ...
        if client_tools:
            ...
            if not db_expecting_tool:
                ...
                if matching_asts:
                    ...
                    _log_tool_chain_snapshot("after_tool_persist_reload", db_msgs, session_id=session_id, enabled=tool_chain_debug, extra=f"persisted_tools={persisted_tools}", log_fn=add_dashboard_log)
            else:
        ...
        _log_tool_chain_snapshot("all_msgs_after_normalize", all_msgs, session_id=session_id, enabled=tool_chain_debug, log_fn=add_dashboard_log)
        ...
        messages = _repair_tool_call_ids_by_adjacency(messages, session_id=session_id, reason="final_messages", log_fn=add_dashboard_log)
        messages = _normalize_tool_chains_by_id(messages)
        messages = _drop_orphan_tool_messages(messages)
        _log_tool_chain_snapshot("final_after_drop_orphan", messages, session_id=session_id, enabled=tool_chain_debug, log_fn=add_dashboard_log)
